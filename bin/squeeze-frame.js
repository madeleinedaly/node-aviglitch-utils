...
const fs = require('fs');
const { ffmpeg, mktmpdir } = require('./helper');
...
(async cli => {
  ...
  const info = await ffmpeg(`-i ${infile}`);
  ...
  if (cli.flags.b) {
    ...
    await ffmpeg(`-loop 1 -f image2 -i ${bfile} -c:v mpeg4 -y -q:v 0 -r ${fps} -t 0.5 -an ${basefile}`);
  } else {
    await ffmpeg(`-i ${infile} -c:v mpeg4 -y -q:v 0 -t 0.5 -r ${fps} -an ${basefile}`);
  }

  const avifile = path.join(tmpdir, 'mpeg4resized.avi');
  const ratio = parseFloat(cli.flags.r);
  const scale = size.map(n => n / ratio).join(':');
  await ffmpeg(`-i ${infile} -c:v mpeg4 -y -q:v 0 -vf scale=${scale} -an ${avifile}`);
  ...
  const glitchfile = path.join('glitch.avi');
  base.ouput(glitchfile);
  const outfile = cli.flags.o;
  if (cli.flags.raw) {
    fs.copyFileSync(glitchfile, outfile);
  } else {
    await ffmpeg(`-i ${infile} -an -q:v 0 -y ${outfile}`);
  }
  process.exit();
})(meow(options));
